...
{
	0x8147 /* 丟 */, 0xB6AA /* 丢 */, // {{{ Simplified-Traditional table from iSearch
	0x814B /* 並 */, 0xB2A2 /* 并 */,
	0x8179 /* 亂 */, 0xC2D2 /* 乱 */,
	0x8183 /* 亙 */, 0xD8A8 /* 亘 */,
	0x8186 /* 亞 */, 0xD1C7 /* 亚 */,
...
void strNormalizeUtf8(string &text, unsigned options)
{
	const char *pTransTable =
			(options & SNO_TO_LOWER) ? _pUpper2Lower : (
			(options & SNO_TO_UPPER) ? _pLower2Upper :
...
void strNormalizeGbk(string &text, unsigned options)
{
	const char *pTransTable =
			(options & SNO_TO_LOWER) ? _pUpper2Lower : (
			(options & SNO_TO_UPPER) ? _pLower2Upper :
...
const string gbkToUtf8(const std::string &text)
{
	string result;
	result.reserve(text.size() * 3 / 2);

	unsigned i_from = 0, flag = 0;
	...
	for (; i_from < text.size(); i_from ++)
	{
		if (flag)
		{
			...
			{
				result += char(0xE0 | (tmp >> 12));
				result += char(0x80 | ((tmp >> 6) & 0x3F));
				result += char(0x80 | (tmp & 0x3F));
			}
			else if (tmp >= 0x80)
			{
				result += char(0xC0 | (tmp >> 6));
				result += char(0x80 | (tmp & 0x3F));
			}
			else
			{
			...
			result += char(text[i_from]);
	}

	return result;
}

const string utf8ToGbk(const std::string &text)
{
	string result;
...
void printToolUsage()
{
	cout << "Usage: ./StrNormalize [-guGUcCas] [f0 f1 ... fn] < input.txt > output.txt" << endl;
	cout << "Options:" << endl;
	cout << "  -h  Help information" << endl;
	cout << "  -g  Source is GBK encoding" << endl;
	cout << "  -u  Source is UTF8 encoding (default)" << endl;
	cout << "  -G  Output is GBK encoding (default if -g)" << endl;
	cout << "  -U  Output is UTF8 encoding (default if -u)" << endl;
	cout << "  -c  Convert upper case to lower case" << endl;
	cout << "  -C  Convert lower case to upper case" << endl;
	cout << "  -a  Convert full width to half width" << endl;
	cout << "  -s  Convert Traditional Chinese to Simplified Chinese" << endl;
	cout << "  fn  Specified fields to be normalized. If not specified, normalize all fields" << endl;
	cout << endl;
	cout << "Last modified at Dec 16, 2010" << endl;
}
...
int main(int argc, char **argv)
{
	...
	bool options[256] = { 0 };
	vector<char> splitFlags; // Collect split field numbers
	for (int i = 1; i < argc; i ++)
	{
		if (argv[i][0] == '-')
			for (int k = 0; k < strlen(argv[i]); k ++)
				options[argv[i][k]] = true;
		else
		{
			for (int k = 0; k < strlen(argv[i]); k ++)
				if (!isdigit(argv[i][k]))
			...
			size_t f = atoi(argv[i]);
			if (splitFlags.size() < f + 1)
				splitFlags.resize(f + 1);
			splitFlags[f] = true;
		}
	}
	if (options['h'] || argc == 1
	...
	if (options['c']) sno |= SNO_TO_LOWER;
	if (options['C']) sno |= SNO_TO_UPPER;
	if (options['a']) sno |= SNO_TO_HALF;
	if (options['s']) sno |= SNO_TO_SIMPLIFIED;

	if (!options['u'] && !options['g']) options['u'] = true;
	if (!options['U'] && !options['G'])
	{
		if (options['u']) options['U'] = true;
		else              options['G'] = true;
	}
	...
	{
		if      (options['u'] && options['G']) line = utf8ToGbk(line);
		else if (options['g'] && options['U']) line = gbkToUtf8(line);

		if (splitFlags.empty() || !sno) // Normalize all fields
		{
			if (sno)
			{
				if (options['U']) strNormalizeUtf8(line, sno);
				else              strNormalizeGbk (line, sno);
			}
			cout << line << endl;
		}
		else
		{
			...
			for (size_t l = 0; l != fields.size(); l++) // Normalize specified fields
			{
				if (l < splitFlags.size() && splitFlags[l])
				{
					if (options['U']) strNormalizeUtf8(fields[l], sno);
					else              strNormalizeGbk (fields[l], sno);
				}
				cout << fields[l];
				if (l != fields.size() - 1)
					cout << "\t";
